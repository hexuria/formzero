...
pub fn hasBusinessCapacity(context: Context) bool {
    return switch (context.subject_kind) {
        ...
        .individual => switch (context.natural_person_classification) {
            ...
            .pure_compensation, .classification_unknown => false,
        },
        .corporation, .partnership, .cooperative, .other_legal_entity => true,
        // Current form evidence does not establish a general business field
        // for estates or trusts. A historical activity row never changes the
        // visibility of the consolidated Base Tax Profile.
        .estate, .trust => false,
    };
}
    ...
    return switch (group) {
        ...
        .trade_name => switch (context.subject_kind) {
            ...
            .individual => switch (context.natural_person_classification) {
                ...
                .classification_unknown => context.has_trade_name,
            },
            .corporation,
            .partnership,
            .cooperative,
        ...
        .line_of_business => hasBusinessCapacity(context),
    };
}
...
test "pure compensation hides business facts regardless of legacy rows" {
    const clean: Context = .{
        .subject_kind = .individual,
...
test "unknown classification cannot expose a hidden legacy activity" {
    const review: Context = .{
        .subject_kind = .individual,
